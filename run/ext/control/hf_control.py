...
G_MODULE_COUNT  = 12    #采样板数量
G_BATTERY_COUNT = 2   #每块采样板电池数量
...
def init_temperature_map():
    ...
    for i in range(0, G_MODULE_COUNT):
            for j in range(0,G_BATTERY_COUNT):  
                if (i*G_BATTERY_COUNT+j)%6==0 and (i*G_BATTERY_COUNT+j) >0: 
                    pos = pos + 1               
                G_TEMPERATURE_MAP[i*G_BATTERY_COUNT+j] = pos 
                print "G_TEMPERATURE_MAP[i*G_BATTERY_COUNT+j]:",i*G_BATTERY_COUNT+j,"pos:",pos
...
for count_battery in range(0, G_MODULE_COUNT * G_BATTERY_COUNT):
    ...
    _battery_kv.append({'skip_save_once': False})
...
def do_tray_release():
    ...
    try:
        ...
        while time.time()-start < 20 :
            time.sleep(0.5)    
            if  G_SENSOR["io"]["IN"]["Gripper_L1"] ==0 and G_SENSOR["io"]["IN"]["Gripper_R1"] ==0 :
                io_out.clear()
...
def do_tray_fastening():
    ...
    while time.time()-start<20:
        time.sleep(0.5)
        if G_SENSOR["io"]["IN"]["Gripper_L1"] == 1 and G_SENSOR["io"]["IN"]["Gripper_R1"] == 1 :
            io_done = True
...
def do_json_command_solver(s_json):
        ...
        for case in switch(action):
                ...
                if case('battery_data'):
                    ...
                    bat_tmp = [0] * (G_MODULE_COUNT * G_BATTERY_COUNT)
                    for i in range(0,G_MODULE_COUNT * G_BATTERY_COUNT):
                       #print str(i) + "==" + str(idx[i]) + "=="  + str(_battery_data[idx[i]]["voltage"])
...
def do_check_go_next_step():
        ...
        if G_PROCESS_IDX > -1:
                ...
                if int(G_PROCESS['process'][G_PROCESS_IDX]['station']) != 6 :
                        ...
                        if stopFlagCount == G_MODULE_COUNT * G_BATTERY_COUNT:
                            ret = True
    ...
    if (G_PROCESS != None) and int(G_PROCESS['process'][G_PROCESS_IDX]['station']) == 6:
        for i in range(0,len(_battery_data)):
            if _battery_data[i]['stop_flag']==0:    
                if _battery_data[i]['voltage'] < -150:
...
def do_contact_check():
    ...
    if ((not G_CONTACT_CHECKED) and (station==1 or station==2 or station==4 or station==3) and (G_CONTACT_CHECK_EXP > 0)):
        ...
        #等待所有电池接触检查完成
        while True and G_PROCESS is not None:
            finishCount = 0
            for midx in range(0,G_MODULE_COUNT):
                #如果这块板没完成接触检查，就继续发命令
...
def do_check_step_stop_condition():
    ...
    for module_index in range(0, G_MODULE_COUNT):
        ...
        if  cmp(_battery_data_stop_flag_check[module_index] , stop_status) != 0:
            G_COMMAND_QUEUE.put(G_PROTOCOL.cmd_Setstop_flagByList(module_index, stop_status))        
...
def do_save_one_battery_point(i):
    ...
    if _battery_kv[i]['skip_save_once']:
        ...
        while True:
                ...
                if G_PROCESS is not None:
                        ...
                        if do_check_go_next_step():
                                ...
                                #清空电流异常计数器
                                for i in range(0,len(G_CURRENT_ERROR_COUNT)):
                                    G_CURRENT_ERROR_COUNT[i]=0
